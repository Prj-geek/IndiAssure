        ...
        <div className="mt-12 hidden sm:block max-w-3xl mx-auto">
          ...
          {rows.map((r) => (
            ...
            >
              ...
              <div className="text-sm text-ink/40 text-center">{r.traditional}</div>
              <div className="text-sm text-ink text-center font-medium">{r.indiassure}</div>
            </div>
          ))}
        </div>
          ...
          {rows.map((r) => (
            <div key={r.label} className="p-4 rounded-xl border border-ink/10 bg-cream/50">
              ...
              <div className="flex justify-between gap-4">
                <div className="text-sm text-ink/40 flex-1">{r.traditional}</div>
                <div className="text-sm text-ink font-medium flex-1 text-right">{r.indiassure}</div>
              </div>
            </div>
          ))}
        </div>
